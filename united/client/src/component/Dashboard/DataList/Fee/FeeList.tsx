...
import { Delete, Edit, Print, Recycling } from '@mui/icons-material';
...
const handledelete =(id:any)=>{
  axios.put('http://localhost:5000/api/fee/trash/'+id)
...
const createFee = async (feeData: any) => {
    ...
    const response = await axios.post('http://localhost:5000/api/fee/create', feeData);
      handleshow()
      dispatch(getAllFeeFn())
...
setStudentId(id)
  axios
    .get(`http://localhost:5000/api/student/get/one/${id}`)
    .then((response) => {
      const { Amount,phone ,Name,method} = response.data;
  ...
  return (
    <div className='Feelistcontiner shadow-[0_3px_10px_rgb(0,0,0,0.2)] m-2 p-2 rounded'>
       ...
       <button onClick={handleshow} className='add'>Add</button> 
       </div>
       </div>
      <table className='tablefee'>
        ...
        <td><input type="text" style={{background:'red'}}   value={ubalance} onChange={(e)=>usetBalance(e.target.value)}/></td>
            ...
            <td className='grid'>
            <button className=''> <Link to={`UpdateFee/${mapFee.id}`}><Edit/></Link></button>
            <button className=''> <Link to={`/print/PrintFee/${mapFee.id}`}><Print/></Link></button>
              <button className='' onClick={()=>handleOpenDeleteDialog(mapFee.id)}><Delete/></button>
            </td>
          </tr>:null
          ))}
        </tbody>
